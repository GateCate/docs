...
| `mode` | `"fork"` \| omit | Optional. Set to `"fork"` to inherit the parent's message history. The compiled graph keeps its own system prompt. See [Forked subagents](/oss/javascript/deepagents/forked-subagents). |
...
A [`CompiledSubAgent`](https://reference.langchain.com/javascript/deepagents/middleware/CompiledSubAgent) can also set `mode: "fork"`. This inherits the parent's message history the same way as a [`ForkedSubAgent`](https://reference.langchain.com/javascript/deepagents/middleware/ForkedSubAgent), but keeps the compiled graph's own system prompt. A `CompiledSubAgent` is already fully built, so it cannot inherit a system message.
...
For more information, see [Forked subagents](/oss/javascript/deepagents/forked-subagents).

## Forked subagents

By default, a subagent sees only the task description you give it and has no memory of the conversation that led up to the delegation. A **forked subagent** inherits the parent's full conversation history and exact system prompt. This is useful when an agent delegates to a subagent deep into an investigation because teh subagent doesn't need to re-derive context the parent already gathered. See [Forked subagents](/oss/javascript/deepagents/forked-subagents).
