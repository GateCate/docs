...
OpenWiki automatically tracks wiki content in `openwiki/.last-update.json` and only rewrites that file when the wiki actually changed. If a scheduled `--update` run does not result in changes, the wiki files stay the same, and the CI does not open a pull reques.
