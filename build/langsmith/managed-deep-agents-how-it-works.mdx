...
When you declare [identity](/langsmith/managed-deep-agents-identity), Managed Deep Agents scopes threads and remounts the matching memory slice for the authenticated user or organization so callers cannot open each other's conversations or memory. Without identity, the deployment uses shared agent memory.
