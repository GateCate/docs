...
The CLI targets US LangSmith Cloud by default. To deploy with an organization-scoped key, set `LANGCHAIN_WORKSPACE_ID` in `.env` or pass `--workspace-id` to `mda deploy`.
...
https://smith.langchain.com/o/<workspace-id>/host/deployments/<deployment-id>
