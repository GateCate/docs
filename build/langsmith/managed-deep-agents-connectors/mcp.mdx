...
Like every connector module, it must export a named `connector` declaration. A TypeScript `export default` fails the build with the fix in the error message.
